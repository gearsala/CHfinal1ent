...
class Cart {
	...
	async add(id: string): Promise<Array<Products>> {
		try {
			...
			const newProduct: Array<Products> = txtFile.filter((product) => product.id === id); // Filtering a single product from product file by passed id
	...
	async delete(id: string): Promise<Array<Products> | -1> {
		try {
			...
			const arrayPosition: number = this.content[0].products.map((product) => product.id).indexOf(id); // Mapping a new array by id if exists returns an the id else returns -1
			const deletedProduct: Array<Products> = this.content[0].products.filter((product) => product.id == id); // Filtering the deleted product into a new Array
